...
(function($w){
  ...
  /*  var _console = (typeof(console) == 'undefined')?{
    ...
    var DEFAULT_OPTIONS = {
        ...
        support_html : true,
	evt_click : null,//evt_click:function (nodeid,node){}
    ...
    jm.node = function(sId,iIndex,sTopic,sUrl,oData,bIsRoot,oParent,eDirection){
    ...
    jm.mind.prototype = {
        ...
        add_node:function(parent_node, nodeid, topic, url, data, idx, direction){
            if(typeof parent_node == 'string'){
                return this.add_node(this.get_node(parent_node), nodeid, topic, url, data, idx, direction);
            }
            var nodeindex = idx || -1;
            if(!!parent_node){
                ...
                if(parent_node.isroot){
                    ...
                    node = new jm.node(nodeid,nodeindex,topic,url, data,false,parent_node,d);
                }else{
                    node = new jm.node(nodeid,nodeindex,topic,url, data,false,parent_node,parent_node.direction);
                }
        ...
        insert_node_before:function(node_before, nodeid, topic, data){
            if(typeof node_before == 'string'){
                return this.insert_node_before(this.get_node(node_before), nodeid, topic, url, data);
            }
            if(!!node_before){
                var node_index = node_before.index-0.5;
                return this.add_node(node_before.parent, nodeid, topic, url, data, node_index);
            }else{
        ...
        insert_node_after:function(node_after, nodeid, topic,url,  data){
            if(typeof node_after == 'string'){
                return this.insert_node_after(this.get_node(node_after), nodeid, topic,url,  data);
            }
            if(!!node_after){
                var node_index = node_after.index + 0.5;
                return this.add_node(node_after.parent, nodeid, topic,url,  data, node_index);
            }else{
    ...
    jm.format = {
        node_tree:{
            ...
            _extract_subnode:function(mind, node_parent, node_json){
                ...
                var node = mind.add_node(node_parent, node_json.id, node_json.topic, node_json.url,data, null, d);
            ...
            _extract_subnode:function(mind, parentid, node_array){
                ...
                while(i--){
                    ...
                    if(node_json.parentid == parentid){
                        ...
                        mind.add_node(parentid, node_json.id, node_json.topic,node_json.url, data, null, d);
                        node_array.splice(i,1);
                        extract_count ++;
		...
		var node_url = xml_node.getAttribute('URL');
                ...
                if(!!parent_id){
                    mind.add_node(parent_id, node_id, node_topic,node_url, node_data, null, node_direction);
                }else{
    ...
    jm.prototype={
	    ...
	    var node = this.get_node(nodeid);
            if(isnode&&node.url&&node.url.length>1){
	    ...
	    if(isnode && nodeid!=null && this.options.evt_click){
		this.options.evt_click(nodeid, node);
	    }
        },
        ...
        add_node:function(parent_node, nodeid, topic,url, data){
            if(this.get_editable()){
                var node = this.mind.add_node(parent_node, nodeid, topic,url, data);
                if(!!node){
        ...
        insert_node_before:function(node_before, nodeid, topic, url, data){
            if(this.get_editable()){
                var node = this.mind.insert_node_before(node_before, nodeid, topic,url,  data);
                if(!!node){
                    this.view.add_node(node);
                    this.layout.layout();
                    this.view.show();
        ...
        insert_node_after:function(node_after, nodeid, topic, url, data){
            if(this.get_editable()){
                var node = this.mind.insert_node_after(node_after, nodeid, topic,url,  data);
                if(!!node){
                    this.view.add_node(node);
                    this.layout.layout();
                    this.view.show();
...
// ============= data provider =============================================
    ...
    jm.view_provider.prototype={
        ...
        expand_size:function(){
            ...
            var client_w = this.e_panel.clientWidth/2;
	    ...
	    d.setAttribute('url',node.url);
	    d.setAttribute('is_click',node.url&&node.url.length>1);
