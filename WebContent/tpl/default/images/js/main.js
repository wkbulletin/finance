...
function init_list_tag(){
	...
	//有确认功能的链接
	$("a[_confirm_date]").click(function (){
		var _confirm_url = $(this).attr("_confirm_date");
		var id = $(this).data("id");
		if(id > 0){
			//alert($("#push_end_t"+id).val());
			var today=new Date();
			...
			var endTime=$("#push_end_t"+id).val()
		    var endTimes   =  endTime.substring(0,10).split('-');  
			var times=endTime.substring(10,19)
		    var mss   =  times.split(':');  
			//  alert('endTimes=='+endTimes[0]+'-'+endTimes[1]+'-'+endTimes[2]+' '+mss[0]+':'+mss[1]+':'+mss[2]); 
			if(year>endTimes[0]){
				alert('当前推送已经失效，请查看结束时间！');
				return false;	
			}
             ...
             if(year>=endTimes[0]&&month>=endTimes[1]&&day>endTimes[2]){
				alert('当前推送已经失效，请查看结束时间！');
				return false;	
			}
            if(year>=endTimes[0]&&month>=endTimes[1]&&day>=endTimes[2]&&hours>mss[0]){
				alert('当前推送已经失效，请查看结束时间！');
				return false;	
			}
               if(year>=endTimes[0]&&month>=endTimes[1]&&day>=endTimes[2]&&hours>=mss[0]&&minutes>mss[1]){
				alert('当前推送已经失效，请查看结束时间！');
				return false;	
			}
               if(year>=endTimes[0]&&month>=endTimes[1]&&day>=endTimes[2]&&hours>=mss[0]&&seconds>mss[2]){
				alert('当前推送已经失效，请查看结束时间！');
				return false;	
			}
		}
