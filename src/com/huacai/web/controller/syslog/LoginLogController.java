...
import com.huacai.web.dao.syslog.MLoginLogDao;

@Controller
@RequestMapping("/syslog")
@Scope("prototype")
public class LoginLogController extends BaseController {
	protected final static Logger logger = LogManager.getLogger(LoginLogController.class);
	private int pageSize = 20;
	@Autowired
	private MLoginLogDao mLoginLogDao;

	@RequestMapping("login_log_list")
	public String login_log_list(Map<String, Object> model) {
		...
		JSONObject logListData = mLoginLogDao.getLogList(pageSize, curPage, queryData);
		...
		if (logListData != null) {
			...
			//model.put("pageStr", PageUtil.page2("/syslog/login_log_list.do", curPage, pageSize, logListData.getIntValue("count"), pageParams));
			PageUtil.page2("/syslog/login_log_list.do", curPage, pageSize, logListData.getIntValue("count"), pageParams,model);
		}
		...
		return "/syslog/login_log_list2";
	}
	@RequestMapping("login_log_export")
	public String login_log_export(Map<String, Object> model) {
		...
		model.putAll(queryData);
		JSONObject logListData = mLoginLogDao.getLogExportList(queryData);

		if (logListData != null) {
			if(logListData.getIntValue("count") == -1){
				return getIframeMessage(model, "导出记录超出最大限制10000条，请按详情条件查询导出", "", "", "/syslog/login_log_list.do", "");
			}
			model.put("info", "");
		...
		exportXls("/syslog/login_log_list.xls", "登录日志.xls", model);
		return null;
	}
}
