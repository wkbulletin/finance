 ...
 * understood by the end user</i>, can change according to
 * "who's looking at it". It's important to understand that a timeline instant,
 * before being presented to the user, <i>must always have an associated time
 * zone - even in the case of a date only, with no time.</i>
 * <li><b>everyday</b> - a date-time in the Gregorian calendar, such as
 * '2009-05-25 18:25:00', which never changes according to
 * "who's looking at it". Here, <i>the time zone is always both implicit and
 * immutable</i>.
 * </ul>
 * 
 * <P>
 ...
 * 
 * In particular, there are a large number of applications which experience <a
 * href='http://martinfowler.com/bliki/TimeZoneUncertainty.html'>problems with
 * time zones</a>, because the timeline model is used instead of the everday
 * model. <i>Such problems are often seen by end users as serious bugs, because
 * telling people the wrong date or time is often a serious issue.</i> <b>These
 * problems make you look stupid.</b>
 * 
 * <a name='JDKDatesMediocre'></a>
 * <h4>Date Classes in the JDK are Mediocre</h4>
 * The JDK's classes related to dates are widely regarded as frustrating to work
 ...
 * The <a href='http://joda-time.sourceforge.net/'>Joda Time</a> library is used
 * by some programmers as an alternative to the JDK classes. Joda Time has the
 * following drawbacks :
 * <ul>
 ...
 * reason.</b>
 * <li>it's large, with well over 100 items in its <a
 * href='http://joda-time.sourceforge.net/api-release/index.html'>javadoc</a>
 ...
 * Civil timekeeping is a byzantine hodge-podge of arcane and arbitrary rules.
 ...
 * <b>Most databases model dates and times using the Gregorian Calendar in an
 * aggressively simplified form</b>, in which :
 * <ul>
 ...
 * database column doesn't usually explicitly store any time zone
 * information</i>.
 * </ul>
 * 
 * <P>
 * <a name='NoTimeZoneInDb'></a>The final point requires elaboration. Some may
 * doubt its veracity, since they have seen date-time information
 * "change time zone" when retrieved from a database. But this sort of change is
 * usually applied using logic which is <i>external</i> to the data stored in
 ...
 * type does <i>not</i> include time zone information (see note <a
 * href='#NoTimeZoneInDb'>above</a>).
 ...
 * unusual items, such as MySQL's <a
 * href='http://dev.mysql.com/doc/refman/5.1/en/time.html'>TIME</a> datatype.
 * 
 * <P>
 ...
 * The {@link #DateTime(String)} constructor accepts a <tt>String</tt>
 * representation of a date-time. The format of the String can take a number of
 * forms. When retrieving date-times from a database, the majority of cases will
 * have little problem in conforming to these formats. If necessary, your SQL
 * statements can almost always use database formatting functions to generate a
 * String whose format conforms to one of the many formats accepted by the
 * {@link #DateTime(String)} constructor.
 * 
 * <p>
 * The {@link #isParseable(String)} method lets you explicitly test if a given
 * String is in a form that can be parsed by this class.
 * 
 * <a name="FormattingLanguage"></a>
 * <h3>Mini-Language for Formatting</h3>
 * This class defines a simple mini-language for formatting a <tt>DateTime</tt>,
 * used by the various <tt>format</tt> methods.
 * 
 * <P>
 * The following table defines the symbols used by this mini-language, and the
 * corresponding text they would generate given the date:
 * 
 * <PRE>
 ...
 * they generate text, not a number.
 * 
 * <P>
 ...
 * When a <tt>DateTime</tt> is passed as a parameter to an SQL statement, the
 * <tt>DateTime</tt> can always be formatted into a <tt>String</tt> of a form
 * accepted by the database, using one of the <tt>format</tt> methods.
 */
public final class DateTime implements Comparable<DateTime>, Serializable {
	...
	/**
	 * Policy for treating 'day-of-the-month overflow' conditions
	 * encountered during some date calculations.
	 * 
	 * <P>
	 ...
	 * 
	 * <P>
	 * What's the answer? Since there is no such thing as June 31, the
	 * result of this operation is inherently ambiguous. This
	 * <tt>DayOverflow</tt> enumeration lists the various policies for
	 * treating such situations, as supported by <tt>DateTime</tt>.
	 * 
	 * <P>
	...
	/**
	 * Constructor taking a date-time as a String. The text is trimmed by
	 * this class.
	 * 
	 * <P>
	 * When this constructor is called, the underlying text can be in an
	 * absolutely arbitrary form, since it will not, initially, be parsed in
	 * any way. This policy of extreme leniency allows you to use dates in
	 * an arbitrary format, without concern over possible transformations of
	 * the date (time zone in particular), and without concerns over
	 * possibly bizarre content, such as '2005-00-00', as seen in some
	 * databases, such as MySQL.
	 * 
	 * <P>
	 * <i>However</i>, the moment you attempt to call <a
	 * href='#TwoSetsOfOperations'>almost any method</a> in this class, an
	 * attempt will be made to parse the given date-time string into its
	 * constituent parts. Then, if the date-time string does not match one
	 * of the example formats listed below, a <tt>RuntimeException</tt> will
	 * be thrown.
	 * 
	 * <P>
	 * Before calling this constructor, you may wish to call
	 * {@link #isParseable(String)} to explicitly test whether a given
	 * String is parseable by this class.
	 * 
	 * <P>
	 * The full date format expected by this class is
	 * <tt>'YYYY-MM-YY hh:mm:ss.fffffffff'</tt>. All fields except for the
	 * fraction of a second have a fixed width. In addition, various
	 * portions of this format are also accepted by this class.
	 * 
	 * <P>
	 ...
	 * with date formats. Since your application is always in control of the
	 * SQL used to talk to the database, you can, if needed, usually use
	 * database format functions to alter the format of dates returned in a
	 * <tt>ResultSet</tt>.
	 */
	...
	/**
	 * Return <tt>true</tt> only if the given String follows one of the
	 * formats documented by {@link #DateTime(String)}.
	 * <P>
	 * If the text is not from a trusted source, then the caller may use
	 * this method to validate whether the text is in a form that's
	 * parseable by this class.
	 */
	 ...
	 *                0..59, optional
	 * @param aNanoseconds
	 *                0..999,999,999, optional (allows for databases that
	 *                store timestamps up to nanosecond precision).
	 */
	...
	/**
	 * Factory method returns a <tt>DateTime</tt> having year-month-day
	 * only, with no time portion.
	 * <P>
	 * See
	 ...
	 * 
	 * <P>
	 * To use nanosecond precision, please use
	 * {@link #forInstantNanos(long, TimeZone)} instead.
	 * 
	 * @param aMilliseconds
	 *                must be in the range corresponding to the range of
	 *                dates supported by this class (year 1..9999);
	 *                corresponds to a millisecond instant on the time-line,
	 *                measured from the epoch used by {@link java.util.Date}
	 *                .
	 */
	...
	/**
	 * For the given time zone, return the corresponding time in
	 * milliseconds-since-epoch for this <tt>DateTime</tt>.
	 * 
	 * <P>
	 * This method is meant to help you convert between a <tt>DateTime</tt>
	 * and the JDK's date-time classes, which are based on the combination
	 * of a time zone and a millisecond value from the Java epoch.
	 * <P>
	 * Since <tt>DateTime</tt> can go to nanosecond accuracy, the return
	 * value can lose precision. The nanosecond value is truncated to
	 * milliseconds, not rounded. To retain nanosecond accuracy, please use
	 * {@link #getNanosecondsInstant(TimeZone)} instead.
	 * <P>
	 ...
	 * 
	 * <P>
	 * To use milliseconds instead of nanoseconds, please use
	 * {@link #forInstant(long, TimeZone)}.
	 * 
	 * @param aNanoseconds
	 *                must be in the range corresponding to the range of
	 *                dates supported by this class (year 1..9999);
	 *                corresponds to a nanosecond instant on the time-line,
	 *                measured from the epoch used by {@link java.util.Date}
	 *                .
	 */
	public static DateTime forInstantNanos(long aNanoseconds, TimeZone aTimeZone) {
		// these items can be of either sign
		long millis = aNanoseconds / MILLION; // integer division
							// truncates towards 0,
							// doesn't round
		long nanosRemaining = aNanoseconds % MILLION; // size 0..999,999
		// when negative: go to the previous millis, and take the
		// complement of nanosRemaining
		if (aNanoseconds < 0) {
			millis = millis - 1;
			nanosRemaining = MILLION + nanosRemaining; // -1
									// remaining
									// coerced
									// to
									// 999,999
		...
		// adjust for nanos - this cast is acceptable, because the
		// value's range is 0..999,999:
	...
	/**
	 * For the given time zone, return the corresponding time in
	 * nanoseconds-since-epoch for this <tt>DateTime</tt>.
	 * 
	 * <P>
	 * For conversion between a <tt>DateTime</tt> and the JDK's date-time
	 * classes, you should likely use {@link #getMilliseconds(TimeZone)}
	 * instead.
	 * <P>
	 * Requires year-month-day to be present; if not, a runtime exception is
	 * thrown.
	...
	public long getNanosecondsInstant(TimeZone aTimeZone) {
		...
		int millis = nanos / MILLION; // integer division truncates,
						// doesn't round
		int nanosRemaining = nanos % MILLION; // 0..999,999 - always
							// positive
		...
		long baseResult = calendar.getTimeInMillis() * MILLION; // either
									// sign
		// the adjustment for nanos is always positive, toward the
		// future:
		return baseResult + nanosRemaining;
	}

	/**
	 * Return the raw date-time String passed to the
	 * {@link #DateTime(String)} constructor. Returns <tt>null</tt> if that
	 * constructor was not called. See {@link #toString()} as well.
	 */
	 ...
	 * <P>
	 * The Modified Julian Day Number is defined by astronomers for
	 * simplifying the calculation of the number of days between 2 dates.
	 * Returns a monotonically increasing sequence number. Day 0 is November
	 * 17, 1858 00:00:00 (whose Julian Date was 2400000.5).
	 * 
	 * <P>
	 ...
	 * 
	 * <P>
	 * (In spite of its name, this method, like all other methods in this
	 * class, uses the proleptic Gregorian calendar - not the Julian
	 * calendar.)
	 * 
	 * <P>
	 * Requires year-month-day to be present; if not, a runtime exception is
	...
	/**
	 * Return an index for the weekday for this <tt>DateTime</tt>. Returns
	...
	/**
	 * Return an integer in the range 1..366, representing a count of the
	 * number of days from the start of the year. January 1 is counted as
	 * day 1.
	 * <P>
	 * Requires year-month-day to be present; if not, a runtime exception is
	 * thrown.
	...
	/**
	 * Return The week index of this <tt>DateTime</tt> with respect to a
	 * given starting <tt>DateTime</tt>.
	 * <P>
	 * The single parameter to this method defines first day of week number
	 * 1. See {@link #getWeekIndex()} as well.
	 * <P>
	 * Requires year-month-day to be present; if not, a runtime exception is
	 * thrown.
	 */
	...
	/**
	 * Return The week index of this <tt>DateTime</tt>, taking day 1 of week
	 * 1 as Sunday, January 2, 2000.
	 * <P>
	 ...
	 * year-month-day as the given parameter. Time is ignored by this
	 * method.
	 * <P>
	 * Requires year-month-day to be present, both for this
	 * <tt>DateTime</tt> and for <tt>aThat</tt>; if not, a runtime exception
	 * is thrown.
	 */
	 ...
	 * <tt>DateTime</tt> comes before the given parameter, according to
	 * {@link #compareTo(DateTime)}, or this <tt>DateTime</tt> equals the
	 * given parameter.
	 */
	...
	/**
	 * 'Greater than or equal to' comparison. Return <tt>true</tt> only if
	 * this <tt>DateTime</tt> comes after the given parameter, according to
	 * {@link #compareTo(DateTime)}, or this <tt>DateTime</tt> equals the
	 * given parameter.
	 */
	 ...
	 * 
	 * @param aPrecision
	 *                takes any value <i>except</i> {@link Unit#NANOSECONDS}
	...
	/**
	 * Return <tt>true</tt> only if all of the given units are present in
	 * this <tt>DateTime</tt>. If a unit is <i>not</i> included in the
	 * argument list, then no test is made for its presence or absence in
	 * this <tt>DateTime</tt> by this method.
	 */
	...
	/**
	 * Return <tt>true</tt> only if this <tt>DateTime</tt> has a non-null
	...
	/**
	 * Return <tt>true</tt> only if all of the given units are absent from
	 * this <tt>DateTime</tt>. If a unit is <i>not</i> included in the
	 * argument list, then no test is made for its presence or absence in
	 * this <tt>DateTime</tt> by this method.
	 */
	 ...
	 * 
	 * <P>
	 * Changes are always applied by this class <i>in order of decreasing
	 * units of time</i>: years first, then months, and so on. After
	 * changing both the year and month, a check on the month-day
	 * combination is made before any change is made to the day. If the day
	 * exceeds the number of days in the given month/year, then (and only
	 * then) the given {@link DayOverflow} policy applied, and the
	 * day-of-the-month is adusted accordingly.
	 * 
	 * <P>
	 ...
	 * 
	 * <P>
	 * <em>The mental model for this method is very similar to that of a car's odometer.</em>
	 * When a limit is reach for one unit of time, then a rollover occurs
	 * for a neighbouring unit of time.
	 * 
	 * <P>
	 ...
	 * <li>year-month-day only. In this case, if a calculation with a time
	 * part is performed, that time part will be initialized by this class
	 * to 00:00:00.0, and the <tt>DateTime</tt> returned by this class will
	 * include a time part.
	 * <li>hour-minute-second (and optional nanoseconds) only. In this case,
	 * the calculation is done starting with the the arbitrary date
	...
	/**
	 * Create a new <tt>DateTime</tt> by subtracting an interval to this
	 * one.
	 * 
	 * <P>
	...
	/**
	 * Return a new <tt>DateTime</tt> by adding an integral number of days
	 * to this one.
	 * 
	 * <P>
	 * Requires year-month-day to be present; if not, a runtime exception is
	 ...
	 * 
	 * @param aNumDays
	 *                can be either sign; if negative, then the days are
	 *                subtracted.
	 */
	public DateTime plusDays(Integer aNumDays) {
		...
		return new DateTime(datePortion.getYear(), datePortion.getMonth(), datePortion.getDay(), fHour,
				fMinute, fSecond, fNanosecond);
	}

	/**
	 * Return a new <tt>DateTime</tt> by subtracting an integral number of
	 * days from this one.
	 * 
	 * <P>
	 * Requires year-month-day to be present; if not, a runtime exception is
	 ...
	 * <P>
	 * Requires year-month-day to be present, both for this
	 * <tt>DateTime</tt> and for the <tt>aThat</tt> parameter; if not, a
	 ...
	 * argument.
	 * <P>
	 * If any date information is present, in either this <tt>DateTime</tt>
	 * or <tt>aThat</tt>, then full year-month-day must be present in
	 * <em>both</em>; if not, then the date portion will be ignored, and
	 * only the time portion will contribute to the calculation.
	 */
	public long numSecondsFrom(DateTime aThat) {
		long result = 0;
		aThat.ensureParsed(); // since the boolean test may
					// short-circuit for aThat
		if (hasYearMonthDay() && aThat.hasYearMonthDay()) {
			result = numDaysFrom(aThat) * 86400; // intermediate
								// value, just
								// the day
								// portion
		}
	...
	/**
	 * Output this <tt>DateTime</tt> as a formatted String using numbers,
	 * with no localizable text.
	 * 
	 * <P>
	 * Example:
	 * 
	 ...
	 * 
	 * <P>
	 * If months, weekdays, or AM/PM indicators are output as localizable
	 * text, you must use {@link #format(String, Locale)}.
	 * 
	 * @param aFormat
	 ...
	 * <P>
	 * If months and weekdays are output as numbers, you are encouraged to
	 * use {@link #format(String)} instead.
	 * 
	 * @param aFormat
	 *                uses the <a href="#FormattingLanguage">formatting
	 *                mini-language</a> defined in the class comment.
	 * @param aLocale
	 *                used to generate text for Month, Weekday and AM/PM
	 *                indicator; required only by patterns which return
	 *                localized text, instead of numeric forms.
	 */
	 ...
	 * 
	 * <P>
	 * Use of this method is likely relatively rare; it should be used only
	 * if the output of {@link #format(String, Locale)} is inadequate.
	 * 
	 * @param aFormat
	 *                uses the <a href="#FormattingLanguage">formatting
	 *                mini-language</a> defined in the class comment.
	 * @param aMonths
	 *                contains text for all 12 months, starting with
	 *                January; size must be 12.
	 * @param aWeekdays
	 *                contains text for all 7 weekdays, starting with
	 *                Sunday; size must be 7.
	 * @param aAmPmIndicators
	 *                contains text for A.M and P.M. indicators (in that
	 *                order); size must be 2.
	 */
	 ...
	 * <P>
	 * Combines the return value of {@link System#currentTimeMillis()} with
	 * the given {@link TimeZone}.
	 * 
	 * <P>
	...
	/**
	 * Return <tt>true</tt> only if this date is in the future, with respect
	 * to {@link #now(TimeZone)}.
	 */
	...
	/**
	 * Return <tt>true</tt> only if this date is in the past, with respect
	 * to {@link #now(TimeZone)}.
	 */
	 ...
	 * 
	 * <P>
	 * A <tt>DateTime</tt> object has an implicit and immutable time zone.
	 * If you need to change the implicit time zone, you can use this method
	 * to do so.
	 * 
	 * <P>
	 * Example :
	 ...
	 * 
	 * <P>
	 * Requires year-month-day-hour to be present; if not, a runtime
	 * exception is thrown.
	 * 
	 * @param aFromTimeZone
	 *                the implicit time zone of this object.
	 * @param aToTimeZone
	 *                the implicit time zone of the <tt>DateTime</tt>
	 *                returned by this method.
	 * @return aDateTime corresponding to the change of time zone implied by
	 *         the 2 parameters.
	 */
	public DateTime changeTimeZone(TimeZone aFromTimeZone, TimeZone aToTimeZone) {
		...
		// other items zeroed out here, since they don't matter for time
		// zone calculations
		fromDate.set(Calendar.SECOND, 0);
		fromDate.set(Calendar.MILLISECOND, 0);

		// millisecond precision is OK here, since the
		// seconds/nanoseconds are not part of the calc
		Calendar toDate = new GregorianCalendar(aToTimeZone);
		toDate.setTimeInMillis(fromDate.getTimeInMillis());
		// needed if this date has hour, but no minute (bit of an
		// oddball case) :
		...
		result = new DateTime(toDate.get(Calendar.YEAR), toDate.get(Calendar.MONTH) + 1,
				toDate.get(Calendar.DAY_OF_MONTH), toDate.get(Calendar.HOUR_OF_DAY), minute,
				getSecond(), getNanoseconds());
		return result;
	}
	 ...
	 * <P>
	 * Uses the 7 date-time elements (year..nanosecond). The Year is
	 * considered the most significant item, and the Nanosecond the least
	 * significant item. Null items are placed first in this comparison.
	 */
	 ...
	 * 
	 * <P>
	 * Equality is determined by the 7 date-time elements
	 * (year..nanosecond).
	 */
	@Override
	public boolean equals(Object aThat) {
		 ...
		 * rejected, since maintaining 'synchronicity' with hashCode
		 * would not then be possible, since hashCode is based only on
		 * one object, not two.
		 */
		ensureParsed();
	 ...
	 * 
	 * <P>
	 * <b>To format this <tt>DateTime</tt> for presentation to the user, see
	 * the various <tt>format</tt> methods.</b>
	 * 
	 * <P>
	 * If the {@link #DateTime(String)} constructor was called, then return
	 * that String.
	 * 
	 * <P>
	 * Otherwise, the return value is constructed from each date-time
	 * element, in a fixed format, depending on which time units are
	 * present. Example values :
	 * <ul>
	 ...
	 * units are present <i>except</i> for minutes, the return value has
	 * this form:
	 * 
	 * <PRE>
	...
	/**
	 * Intended as internal tool, for testing only. Note scope is not
	 * public!
	 */
	void ensureParsed() {
		if (!fIsAlreadyParsed) {
	...
	/**
	 * Return the number of days in the given month. The returned value
	 * depends on the year as well, because of leap years. Returns
	 * <tt>null</tt> if either year or month are absent. WRONG - should be
	 * public?? Package-private, needed for interval calcs.
	 */
	...
	/*
	 * There are 2 representations of a date - a text form, and a 'parsed'
	 * form, in which all of the elements of the date are separated out. A
	 * DateTime starts out with one of these forms, and may need to generate
	 * the other.
	 */
	...
	/**
	 * Return the number of seconds in any existing time portion of the
	 * date.
	 */
	...
	private void checkRange(Integer aValue, int aMin, int aMax, String aName) {
		if (aValue != null) {
			if (aValue < aMin || aValue > aMax) {
				throw new ItemOutOfRange(aName + " is not in the range " + aMin + ".." + aMax
						+ ". Value is:" + aValue);
			}
		}
	}
	...
	private void checkNumDaysInMonth(Integer aYear, Integer aMonth, Integer aDay) {
		if (hasYearMonthDay(aYear, aMonth, aDay) && aDay > getNumDaysInMonth(aYear, aMonth)) {
			throw new ItemOutOfRange("The day-of-the-month value '" + aDay
					...
					+ getNumDaysInMonth(aYear, aMonth));
		}
	}
	...
	private void parseDateTimeText() {
		...
		/*
		 * This is unusual - we essentially copy from one object to
		 * another. This could be avoided by building another interface,
		 * But defining a top-level interface for this simple task is
		 * too high a price.
		 */
	...
	private String calcToStringFormat() {
		String result = null; // caller will check for this; null means
					// the set of units is bizarre
		if (unitsAllPresent(Unit.YEAR)
				&& unitsAllAbsent(Unit.MONTH, Unit.DAY, Unit.HOUR, Unit.MINUTE, Unit.SECOND,
						Unit.NANOSECONDS)) {
			result = "YYYY";
