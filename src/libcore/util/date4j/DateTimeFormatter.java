...
final class DateTimeFormatter {
	...
	/**
	 * Constructor used for patterns that represent date-time elements using
	 * not only numbers, but text as well. The text needs to be localizable.
	 * 
	 * @param aFormat
	 *                uses the syntax described by
	 *                {@link DateTime#format(String)}.
	 ...
	 *                localized text, instead of numeric forms for date-time
	 *                elements.
	 */
	...
	/**
	 * Constructor used for patterns that represent using not only numbers,
	 * but customized text as well.
	 * 
	 * <P>
	 * This constructor exists mostly since SimpleDateFormat doesn't support
	 * all locales, and it has a policy of N letters for text, where N != 3.
	 * 
	 * @param aFormat
	 ...
	 * populated only when a specific Locale is needed for presenting such
	 * text. Used for MMMM and MMM tokens.
	 */
	 ...
	 * populated only when a specific Locale is needed for presenting such
	 * text. Used for WWWW and WWW tokens.
	 */
		...
		CustomLocalization(List<String> aMonths, List<String> aWeekdays, List<String> aAmPm) {
			if (aMonths.size() != 12) {
				throw new IllegalArgumentException(
						"Your List of custom months must have size 12, but its size is "
								+ aMonths.size());
			}
			if (aWeekdays.size() != 7) {
				throw new IllegalArgumentException(
						"Your List of custom weekdays must have size 7, but its size is "
								+ aWeekdays.size());
			}
			if (aAmPm.size() != 2) {
				throw new IllegalArgumentException(
						"Your List of custom a.m./p.m. indicators must have size 2, but its size is "
								+ aAmPm.size());
			}
			Months = aMonths;
			Weekdays = aWeekdays;
	...
	/**
	 * A section of fFormat bounded by a pair of escape characters; such
	 * ranges contain uninterpreted text.
	 */
	...
	/**
	 * Special character used to escape the interpretation of parts of
	 * fFormat.
	 */
	...
	/*
	 * Here, 'token' means an item in the mini-language, having special
	 ...
	 * 
	 * 12:00 am is midnight, 12:30am is 30 minutes past midnight, 12:00 pm
	 * is 12 noon. This item is almost always used with 'a' to indicate
	 * am/pm.
	 */
	...
	private static final int AM = 0; // a.m. comes first in lists used by
						// this class
	...
	/**
	 * The order of these items is significant, and is critical for how
	 * fFormat is interpreted. The 'longer' tokens must come first, in any
	...
	/**
	 * Escaped ranges are bounded by a PAIR of {@link #ESCAPE_CHAR}
	 * characters.
	 */
	...
	/**
	 * Return true only if the start of the interpreted range is in an
	 * escaped range.
	 */
	private boolean isInEscapedRange(InterpretedRange aInterpretedRange) {
		...
		for (EscapedRange escapedRange : fEscapedRanges) {
			// checking only the start is sufficient, because the
			// tokens never contain the escape char
			if (escapedRange.Start <= aInterpretedRange.Start
					&& aInterpretedRange.Start <= escapedRange.End) {
				result = true;
				break;
			}
		}
	...
	/**
	 * Scan fFormat for all tokens, in a specific order, and interpret them
	 * with the given DateTime. The interpreted tokens are saved for output
	 * later.
	 */
	...
	private String withCharDenotingAlreadyInterpreted(String aToken) {
		...
		for (int idx = 1; idx <= aToken.length(); ++idx) {
			// any character that isn't interpreted, or a special
			// regex char, will do here
			// the fact that it's interpreted at location x is
			// stored elsewhere;
			// this is meant only to prevent multiple
	...
	private String lookupMonthFor(Integer aMonth) {
		...
		result = fMonths.get(fLocale).get(aMonth - 1); // list is
								// 0-based
		return result;
	}
	...
	private String lookupWeekdayFor(Integer aWeekday) {
		...
		if (!fWeekdays.containsKey(fLocale)) {
			...
			for (int idx = 8; idx <= 14; ++idx) {
				...
				firstDayOfWeek.set(Calendar.MONTH, 1); // month
									// is
									// 0-based
		...
		result = fWeekdays.get(fLocale).get(aWeekday - 1); // list is
									// 0-based
		return result;
	}
