...
import libcore.util.date4j.DateTime.DayOverflow;
import libcore.util.date4j.DateTime.Unit;
...
final class DateTimeInterval {
	...
	// work area for the final result - starts off with values from base
	// date fFrom
	...
	private void checkRangeNanos(Integer aValue) {
		if (aValue < MIN_NANOS || aValue > MAX_NANOS) {
			throw new IllegalArgumentException("Nanosecond interval is not in the range " + MIN_NANOS
					+ ".." + MAX_NANOS);
		}
	}
	 ...
	 * time. They are just added. If they under/over flow, then extra math
	 * is performed. They don't over/under by more than 1 second, since the
	 * size of the increment is limited.
	 */
