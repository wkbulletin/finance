...
final class DateTimeParser {
	 ...
	 * <tt>DateTime</tt>, since it doesn't have a format allowed by this
	 ...
	 * Group index VARIES for y-m-d according to which option is selected
	 * Year: Group 1, 4, 6 Month: Group 2, 5 Day: Group 3
	 */
	private static final Pattern DATE = Pattern
			.compile("(\\d{1,4})-(\\d\\d)-(\\d\\d)|(\\d{1,4})-(\\d\\d)|(\\d{1,4})");
	...
	private static final String TT = "(\\d\\d)"; // colon is a special
							// character
	...
	private static final Pattern TIME = Pattern.compile("" + TT + CL + TT + CL + TT + "\\." + "(\\d{1,"
			+ NUM_DIGITS_FOR_FRACTIONAL_SECONDS + "})" + "|" + TT + CL + TT + CL + TT + "|" + TT + CL + TT
			+ "|" + TT);
	...
	/**
	 * Convert any number of decimals (1..9) into the form it would have
	 * taken if nanos had been used, by adding any 0's to the right side.
	 */
