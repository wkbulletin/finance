 ...
 * <P>
 * <b>toString()</b><br>
 * This class is intended for the most common case, where <tt>toString</tt> is
 * used in an <em>informal</em> manner (usually for logging and stack traces).
 * That is, <span class="highlight"> the caller should not rely on the
 * <tt>toString()</tt> text returned by this class to define program
 * logic.</span>
 * 
 * <P>
 * Typical example :
 ...
 * returns a <tt>Product</tt> : instead of calling <tt>Product.toString()</tt>,
 ...
 * <P>
 * If an object is <a
 * href="http://www.javapractices.com/Topic29.cjp">immutable</a>, then the
 ...
 * </PRE>
 * 
 * See note above regarding <a
 * href="#GetSignificantFields">getSignificantFields()</a>.
 * 
 * <P>
 ...
 * 	if (this == aThat)
 * 		return true;
 * 	if (!(aThat instanceof Planet))
 * 		return false;
 * 	Planet that = (Planet) aThat;
 * 	return EqualsUtil.areEqual(this.fPossiblyNullObject, that.fPossiblyNullObject)
 * 			&amp;&amp; EqualsUtil.areEqual(this.fCollection, that.fCollection)
 * 			&amp;&amp; EqualsUtil.areEqual(this.fPrimitive, that.fPrimitive)
 * 			&amp;&amp; Arrays.equals(this.fArray, that.fArray); // arrays
 * 									// are
 * 									// different!
 * }
 * </PRE>
 ...
 * hirondelle.web4j.model.ModelUtil.NullsGo)'>comparePossiblyNull</a>, (where
...
final class ModelUtil {
	...
	/**
	 * Implements an override of <tt>Object.toString()</tt> (see class
	 * comment).
	 * 
	 * <P>
	 * Example output format, for an <tt>Rsvp</tt> object with 4 fields :
	 * 
	 * <PRE>
	 ...
	 * </PRE>
	 * 
	 * (There is no indentation since it causes problems when there is
	 * nesting.)
	 * 
	 * <P>
	 ...
	 * <li>factory methods which return an object of the native class ("
	 * <tt>getInstance()</tt>" methods)
	 * </ul>
	 * 
	 * <P>
	 * Reflection is used to access field values. Items are converted to a
	 * <tt>String</tt> simply by calling their <tt>toString method</tt>,
	 * with the following exceptions :
	 * <ul>
	 * <li>for arrays, the {@link Util#getArrayAsString(Object)} is used
	 * <li>for methods whose name contains the text <tt>"password"</tt>
	 * (case-insensitive), their return values hard-coded to '<tt>****</tt>
	 * '.
	 * </ul>
	 * 
	 * <P>
	 * If the method name follows the pattern '<tt>getXXX</tt>', then the
	 * word '<tt>get</tt>' is removed from the result.
	 * 
	 * <P>
	 * <span class="highlight">WARNING</span>: If two classes have cyclic
	 * references (that is, each has a reference to the other), then
	 * infinite looping will result if <em>both</em> call this method! To
	 * avoid this problem, use <tt>toStringFor</tt> for one of the classes,
	 * and {@link #toStringAvoidCyclicRefs} for the other class.
	 * 
	 * @param aObject
	 *                the object for which a <tt>toString()</tt> result is
	 *                required.
	 */
	...
	/**
	 * As in {@link #toStringFor}, but avoid problems with cyclic
	 * references.
	 * 
	 * <P>
	 * Cyclic references occur when one Model Object references another, and
	 * both Model Objects have their <tt>toString()</tt> methods implemented
	 * with this utility class.
	 * 
	 * <P>
	 * Behaves as in {@link #toStringFor}, with one exception: for methods
	 * of <tt>aObject</tt> that return instances of <tt>aSpecialClass</tt>,
	 * then call <tt>aMethodName</tt> on such instances, instead of
	 * <tt>toString()</tt>.
	 */
	 ...
	 * 
	 * <P>
	 * Each element of <tt>aFields</tt> must be an {@link Object}, or an
	 * array containing possibly-null <tt>Object</tt>s. These items will
	 * each contribute to the result. (It is not a requirement to use
	 * <em>all</em> fields related to an object.)
	 * 
	 * <P>
	 ...
	 * 
	 * Contributions from individual fields are 'added' to this initial
	 * value. (Using a non-zero value decreases collisons of
	 ...
	 * 
	 * <P>
	 * <tt>aObject</tt> is a possibly-null object field, and possibly an
	 * array.
	 * 
	...
	/**
	 * Quick checks for <em>possibly</em> determining equality of two
	 * objects.
	 * 
	 * <P>
	 * This method exists to make <tt>equals</tt> implementations read more
	 * legibly, and to avoid multiple <tt>return</tt> statements.
	 * 
	 * <P>
	 * <em>It cannot be used by itself to fully implement <tt>equals</tt>. </em>
	 * It uses <tt>==</tt> and <tt>instanceof</tt> to determine if equality
	 * can be found cheaply, without the need to examine field values in
	 * detail. It is <em>always</em> paired with some other method (usually
	 * {@link #equalsFor(Object[], Object[])}), as in the following example
	 * :
	 * 
	 * <PRE>
	 *    public boolean equals(Object aThat){
	 ...
	 * Here, <tt>true</tt> and <tt>false</tt> mean that a simple quick check
	 * was able to determine equality. <span class='highlight'>The
	 * <tt>null</tt> case means that the quick checks were not able to
	 * determine if the objects are equal or not, and that further
	 * field-by-field examination is necessary. The caller must always
	 * perform a check-for-null on the return value.</span>
	 */
	 ...
	 * 
	 * <P>
	 * Both <tt>Object[]</tt> parameters are the same size. Each includes
	 * all fields that have been deemed by the caller to contribute to the
	 * <tt>equals</tt> method. <em>None of those fields are 
   array fields.</em> The order is the same in
	 * both arrays, in the sense that the Nth item in each array corresponds
	 * to the same underlying field. The caller controls the order in which
	 * fields are compared simply through the iteration order of these two
	 * arguments.
	 * 
	 * <P>
	...
	static boolean equalsFor(Object[] aThisSignificantFields, Object[] aThatSignificantFields) {
		...
		if (aThisSignificantFields.length != aThatSignificantFields.length) {
			throw new IllegalArgumentException("Array lengths do not match. 'This' length is "
					+ aThisSignificantFields.length + ", while 'That' length is "
	 ...
	 * 
	 * <P>
	 * Note that <tt>byte</tt>, <tt>short</tt>, and <tt>int</tt> are handled
	...
	/**
	 * Define hows <tt>null</tt> items are treated in a comparison. Controls
	 * if <tt>null</tt> items appear first or last.
	 * 
	 * <P>
	 * See <a href='#comparePossiblyNull(T, T,
	 * hirondelle.web4j.model.ModelUtil.NullsGo)'>comparePossiblyNull</a>.
	 */
	enum NullsGo {
		FIRST, LAST
	}
	...
	/**
	 * Utility for implementing {@link Comparable}. See <a
	 * href='#Comparable'>class example</a> for illustration.
	 * 
	 * <P>
	 ...
	 * <em>without</em> throwing a {@link NullPointerException}. In this
	 * way, this method can handle nullable fields just like any other
	 * field.
	 * 
	 * <P>
	 * There are <a href=
	 ...
	 * @param aNullsGo
	 *                defines if <tt>null</tt> items should be placed first
	 *                or last
	 */
